        ...
        if (event.getTargetEntity().getType() == EntityTypes.PLAYER) {
            ...
            if (effectBase != null && !event.getCause().first(SoulBindEffect.class).isPresent()) {
                ...
                if (effect.getConsumer() == character) {
                   effect.getTarget().getEntity().damage(event.getBaseDamage(), builder.build() ,Cause.of(effect));
                } else {
                    effect.getConsumer().getEntity().damage(event.getBaseDamage(),builder.build(),Cause.of(effect));
                }
            }
        }
    }
